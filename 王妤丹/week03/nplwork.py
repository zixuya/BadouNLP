...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, hidden_size, sentence_length, vocab):
        ...
        self.layer = nn.RNN(input_size=vector_dim, hidden_size=hidden_size, bias=False, batch_first=True)  #rnn
    ...
    def forward(self, x, y=None):
        ...
        output, hn  = self.layer(x)                          #（batch_size, vector_dim, 1)
        # x = x.squeeze()                            #(batch_size, vector_dim, 1) -> (batch_size, vector_dim)
        x = self.classify(output[:, -1, :])                       #(batch_size, vector_dim) -> (batch_size, 1) 3*5 5*1 -> 3*1
...
def build_dataset(sample_length, vocab, sentence_length):
    ...
    for i in range(sample_length):
        ...
        dataset_x.append(x)
        dataset_y.append([y])
    print("dataset_x", dataset_x, "dataset_y", dataset_y)
