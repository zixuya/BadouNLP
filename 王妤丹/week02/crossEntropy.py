class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        self.activation = torch.argmax  # sigmoid归一化函数
    ...
    def forward(self, x, y=None):
        ...
        if y is not None:
            ...
            return self.activation(x, dim=1)  # dim=1 返回每行最大值的索引 ps：dim=0 返回每列最大值的索引）
...
def main():
    ...
    for epoch in range(epoch_num):
        ...
        for batch_index in range(train_sample // batch_size):
            ...
            loss = model(x, y.long())  # 计算loss  model.forward(x,y)
            loss.backward()  # 计算梯度
            optim.step()  # 更新权重
