...
class SiameseNetwork(nn.Module):
    ...
    def cosine_triplet_loss(self, a, p, n, margin=None):
        ...
        if margin is None:
            ...
            diff = ap - an + margin
        return torch.mean(torch.clamp(diff, min = 0)) # diff[diff.gt(0)]
    ...
    def forward(self, a, p = None, n = None, margin = 0.1):
        #同时传入a,p,n,计算triplet loss
        if p is not None and n is not None:
            a_vec = self.sentence_encoder(a) #vec:(batch_size, hidden_size)
            p_vec = self.sentence_encoder(p) 
            n_vec = self.sentence_encoder(n)
            
            return self.cosine_triplet_loss(a_vec, p_vec, n_vec, margin)
        #单独传入一个句子时，认为正在使用向量化能力
        else:
