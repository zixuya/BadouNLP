...
class TorchModel(nn.Module):
    def __init__(self, config):
        ...
        if model_type == "fast_text":  # fast_text模型,直接使用embedding层,不需要encoder,这里lambda x: x表示直接返回x，只是为了保证代码结构
    ...
    def forward(self, x, target=None):
        if self.use_bert:  # bert返回的结果是 (sequence_output, pooler_output)
            ...
            # bert和其它模型最大区别在于它内部是包含了embedding层的，所以输入x不需要再进行embedding
            x = self.encoder(x)
        else:
...
class GatedCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        b = self.gate(x)
        b = torch.sigmoid(b)
        return torch.mul(a, b)  # torch.mul()逐元素相乘
...
class BertCNN(nn.Module):
    ...
    def forward(self, x):
        x = self.bert(x)[0]  # (batch_size, max_len, embeding_size)
        x = self.cnn(x)      
        return x

# Bert中间层：bert的输出是tuple，第一个是最后一层的输出，第二个是pooler的输出，第三个是所有层的输出
# 这种方式可以获取所有层的输出，然后取倒数第二层和最后一层的输出相加
# 但不推荐，一般设置bert的num_hidden_layers=3，这样就可以将前面几层的输出加起来
class BertMidLayer(nn.Module):
    def __init__(self, config):
        super(BertMidLayer, self).__init__()
        self.bert = BertModel.from_pretrained(config["pretrain_model_path"], return_dict=False)
        self.bert.config.output_hidden_states = True  # output_hidden_states这个参数表示是否输出中间层的结果

    def forward(self, x):
        # bert的输出是tuple，第一个是最后一层的输出，第二个是pooler的输出，第三个是所有层的输出
        # output_hidden_states参数为True时可以获取[2]的输出，就是所有层的输出
        layer_states = self.bert(x)[2]  # (13, batch, len, hidden)
        layer_states = torch.add(layer_states[-2], layer_states[-1])  # 取倒数第二层和最后一层的输出相加
        return layer_states
