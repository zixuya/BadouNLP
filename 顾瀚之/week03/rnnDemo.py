...
#建立模型
def build_model(vocab, char_dim, sentence_length, hiden_size):
    model = TorchModel(char_dim, sentence_length, vocab, hiden_size)
    return model
...
def main():
    ...
    vocab = build_vocab()
    # 建立模型
    model = build_model(vocab, char_dim, sentence_length, hiden_size)
    # 选择优化器
...
def predict(model_path, vocab_path, input_strings):
    ...
    model = build_model(vocab, char_dim, sentence_length, hiden_size)  #建立模型
    ...
    for i, input_string in enumerate(input_strings):
        print("输入：%s, 概率值：%s" % (input_string, result[i]))  #打印结果


if __name__ == "__main__":
    main()
    test_strings = ["fnvfee", "wz你df你", "rqwdeg", "n我kwww"]
