...
class SiameseNetwork(nn.Module):
    ...
    def forward(self, anchor, positive=None, negtive=None):
        #同时传入两个句子
        if positive is not None:
            vector1 = self.sentence_encoder(anchor) #vec:(batch_size, hidden_size)
            vector2 = self.sentence_encoder(positive)
            vector3 = self.sentence_encoder(negtive)
