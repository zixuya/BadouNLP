...
class FindIndexModel(nn.Module):
    def __init__(self,vocab,dim_size,sentence_length):
        ...
        self.rnn =  nn.RNN(dim_size, sentence_length, bias=False, batch_first=True)  
        self.pool = nn.AvgPool1d(sentence_length+1) # 池化
    ...
    def forward(self,x,y=None):
        ...
        x , h = self.rnn(x) # shape = batch_size ,sentence_length,sentence_length
        x =  self.linear(x) # shape = batch_size ,sentence_length,sentence_length+1
        x = self.pool(x)  # shape = batch_size ,sentence_length,1
        x = torch.squeeze(x)  # shape = batch_size ,sentence_length
        pred =  self.linear(x) # shape = batch_size ,sentence_length+1
        if y is not None:
...
if __name__ == '__main__':
   train()
   print("=====开始预测结果=====")
   test_strings = ["张wzsd", "w张zsdf",  "nc张ww","ncw张w","ds","ncxcc"]
