...
class DailogueSystem:
    ...
    def nlu(self, memory):
        # 将当前查询和历史对话合并用于意图识别
        combined_query = self.combine_history_with_query(memory)
        memory["combined_query"] = combined_query
        memory = self.intent_recognition(memory)
        memory = self.slot_filling(memory)
        return memory

    def combine_history_with_query(self, memory):
        # 将历史对话和当前查询组合在一起
        history = memory.get("dialogue_history", [])
        current_query = memory["query"]
        combined = current_query
        if history:
            # 只使用最近的3轮对话历史
            recent_history = history[-3:]
            combined = " ".join([f"{'User' if i % 2 == 0 else 'System'}: {turn}" for i, turn in enumerate(recent_history)])
            combined += f" User: {current_query}"
        return combined
    ...
    def intent_recognition(self, memory):
        ...
        for node_name in memory["available_nodes"]:
            ...
            score = self.get_node_score(memory["combined_query"], node_info)
            if score > max_score:
                max_score = score
    ...
    def slot_filling(self, memory):
        ...
        for slot in slot_list:
            ...
            if re.search(slot_values, memory["combined_query"]):
                memory[slot] = re.search(slot_values, memory["combined_query"]).group()
        return memory
    ...
    def generate_response(self, query, memory):
        # 初始化对话历史（如果不存在）
        if "dialogue_history" not in memory:
        ...
        # 更新对话历史
        memory["dialogue_history"].append(query)
        memory["dialogue_history"].append(memory["response"])
