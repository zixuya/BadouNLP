...
# rnn\birnn\lstm\bilstm效果都一般，bilstm特定学习率的时候可能会到0.8，bert最好，不训练就有0.8，训练能到0.9
def main(config):
    train_data, data_dict = loader(config["train_data_path"], config)
    model = All_Models(config)
    optim = choose_optim(model, config)
    eval = Evaluation(config, model, data_dict)
    ...
    for epoch in range(config["epochs"]):
        ...
        for index, batch_data in enumerate(train_data):
            ...
            train_anchor, train_pos, train_neg = batch_data
            train_neg = train_neg.transpose(1, 2).squeeze(-1)
            loss = model(train_anchor, train_pos, train_neg)
            loss.backward()
            optim.step()
            optim.zero_grad()
