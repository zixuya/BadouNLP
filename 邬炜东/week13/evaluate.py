...
class Evaluation:
    ...
    def decode(self, sentence):
        ...
        for location in re.finditer("(04+)", strs):
            ...
            result.append(''.join(str(i) for i in range(start, end)))
            result_location.append(''.join(str(i) for i in range(start, end)))

        for location in re.finditer("(15+)", strs):
            start, end = location.span()
            result.append(''.join(str(i) for i in range(start, end)))
            result_organization.append(''.join(str(i) for i in range(start, end)))

        for location in re.finditer("(26+)", strs):
            start, end = location.span()
            result.append(''.join(str(i) for i in range(start, end)))
            result_person.append(''.join(str(i) for i in range(start, end)))

        for location in re.finditer("(37+)", strs):
            start, end = location.span()
            result.append(''.join(str(i) for i in range(start, end)))
            result_time.append(''.join(str(i) for i in range(start, end)))
