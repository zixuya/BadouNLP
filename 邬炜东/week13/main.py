...
def main(config):
    ...
    if config["is_lora"]:
        ...
        for param in model.classify.parameters():
            param.requires_grad = True
        # trainable_param_keys = [name for name, param in model.named_parameters() if param.requires_grad]
        # print(trainable_param_keys)
    ...
    plt.legend()
    plt.show()
    save_trainable_parameters(model, config["model_path"])
