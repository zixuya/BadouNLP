...
class DataGenerator:
    ...
    def __getitem__(self, index):
        if self.data_type == "train":
            ...
            return self.triplet_loss_sample()
        else:
    ...
    def triplet_loss_sample(self):
        standard_question_index = list(self.knwb.keys())
        i,j = random.sample(standard_question_index, 2)
        if len(self.knwb[i]) < 2:
            return self.triplet_loss_sample()
        else:
            a, p = random.sample(self.knwb[i], 2)
            n = random.choice(self.knwb[j])
            return [a, p, n]
...
if __name__ == "__main__":
    ...
    # dg = DataGenerator(r"week8 文本匹配问题/data/valid.json", Config)
    # print(dg[2])

    # text = "畅聊套餐变更"
    # input_id = []
    # vocab = load_vocab(r"week8 文本匹配问题/chars.txt")
    # for word in jieba.cut(text):
    # # for char in text:
    #     input_id.append(vocab.get(word, vocab["[UNK]"]))
    # input_id = input_id[:20]
    # print(input_id)
    # input_id += [0] * (20 - len(input_id))
    # print(input_id)

    path = r"week8 文本匹配问题/data/schema.json"
    print(load_schema(path))
