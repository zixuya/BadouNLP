...
class DailogueSystem:
    ...
    def load(self):
        self.nodes_info = {}
        self.file_path = "/Users/jessicachan/nlp20/week16_dailogue_system/scenario-买衣服.json"
        self.load_scenario(self.file_path)
        #self.load_scenario("scenario-看电影.json")
        self.load_slot_template("/Users/jessicachan/nlp20/week16_dailogue_system/slot_fitting_templet.xlsx")
    
    def load_scenario(self, scenario_file):
        with open(scenario_file, "r", encoding="utf-8") as f:
            self.scenario = json.load(f)
        print(scenario_file)
        #取出/Users/jessicachan/nlp20/week16_dailogue_system/scenario-买衣服.json中文件名
    ...
    def dpo(self, memory):
        ...
        if memory["require_slot"] is None:
            ...
            memory["policy"] = "ask"
            memory["repeat_query"] = re.search("重复一遍｜再说一遍｜没听清", memory["query"])
            if memory["repeat_query"]: #如果用户说重复一遍，则重复之前的问题
                memory["available_nodes"] = [memory["hit_node"]]
            memory["available_nodes"] = [memory["hit_node"]] #停留在当前节点，直到槽位填满
        return memory

    def nlg(self, memory):
        #根据policy生成回复,反问、回复、重复上面的问题
