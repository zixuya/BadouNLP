...
class DiyBert:
    ...
    def load_weights(self, state_dict):
        ...
        self.position_embeddings = state_dict[
            "embeddings.position_embeddings.weight"].numpy()  # [max_len 512,hidden_size 768]
        self.token_type_embeddings = state_dict[
            "embeddings.token_type_embeddings.weight"].numpy()  # [2,hidden_size 768]
        ...
        for i in range(self.num_layers):
            q_w = state_dict[
                "encoder.layer.%d.attention.self.query.weight" % i].numpy()  # [hidden_size 768,hidden_size 768]
            q_b = state_dict["encoder.layer.%d.attention.self.query.bias" % i].numpy()  # [hidden_size 768]
            k_w = state_dict[
                "encoder.layer.%d.attention.self.key.weight" % i].numpy()  # [hidden_size 768,hidden_size 768]
            k_b = state_dict["encoder.layer.%d.attention.self.key.bias" % i].numpy()  # [hidden_size 768]
            v_w = state_dict[
                "encoder.layer.%d.attention.self.value.weight" % i].numpy()  # [hidden_size 768,hidden_size 768]
            v_b = state_dict["encoder.layer.%d.attention.self.value.bias" % i].numpy()  # [hidden_size 768]
            attention_output_weight = state_dict[
                "encoder.layer.%d.attention.output.dense.weight" % i].numpy()  # [hidden_size 768,hidden_size 768]
            attention_output_bias = state_dict[
                "encoder.layer.%d.attention.output.dense.bias" % i].numpy()  # [hidden_size 768]
            attention_layer_norm_w = state_dict[
                "encoder.layer.%d.attention.output.LayerNorm.weight" % i].numpy()  # [hidden_size 768]
            attention_layer_norm_b = state_dict[
                "encoder.layer.%d.attention.output.LayerNorm.bias" % i].numpy()  # [hidden_size 768]
            intermediate_weight = state_dict[
                "encoder.layer.%d.intermediate.dense.weight" % i].numpy()  # [intermediate_size 3072,hidden_size 768] 转至得换位置
            intermediate_bias = state_dict[
                "encoder.layer.%d.intermediate.dense.bias" % i].numpy()  # [intermediate_size 3072]
            output_weight = state_dict[
                "encoder.layer.%d.output.dense.weight" % i].numpy()  # [hidden_size 768,intermediate_size 3072]  转至得换位置
    ...
    # 最终输出
    def forward(self, x):
        x = self.embedding_forward(x)  # [max_len 4, hidden_size 768]
        sequence_output = self.all_transformer_layer_forward(x)  # [max_len, hidden_size 768]
        pooler_output = self.pooler_output_layer(sequence_output[0])  # [hidden_size 768]
