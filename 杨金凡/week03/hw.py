...
def build_sample(vocab, sentence_length):
    ...
    seen = set()
    x = [word for word in x if word not in seen and not seen.add(word)]
    if '我' in x:
        y = x.index('我')
    else:
        y = sentence_length
...
def predict(model_path, vocab_path, input_strings):
    ...
    model.load_state_dict(torch.load(model_path,weights_only=True),strict=False)             #加载训练好的权重
    x = []
...
if __name__ == "__main__":
    main()
    test_strings = ["f我jfee", "mn你dfg", "ikld我g", "我hhjn他"]
