...
class SiameseNetwork(nn.Module):
    ...
    def cosine_triplet_loss(self, a, p, n, margin=None):
        ...
        if diff[diff > 0].numel() == 0:  # 空张量的情况
            return torch.tensor(0.0, requires_grad=True).to(a.device)
    ...
    # sentence: (batch_size, max_length)
    # 支持输入三元组 (anchor, positive, negative) 或单句子
    def forward(self, anchor, positive=None, negative=None):
        if positive is not None and negative is not None:
            ...
            # 计算 Triplet Loss
            loss = self.cosine_triplet_loss(anchor_vec, positive_vec, negative_vec)
            return loss

        # 如果只输入 anchor，则返回其向量表示
        else:
