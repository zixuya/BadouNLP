...
def evaluate(model, vocab, sample_length, device):
    model.eval()
    x, y = build_dataset(2000, vocab, sample_length)   #建立2000个用于测试的样本
    x, y = x.to(device), y.to(device)
    print("本次预测集中共有%d个正样本，%d个负样本"%(sum(y), 2000 - sum(y)))
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)  # 模型预测
...
def train():
    ...
    vocab = build_vocab()
    # 建立模型
    model = build_model(vocab, char_dim, sentence_length).to(device)
    # 选择优化器
...
#使用训练好的模型做预测
def predict(model_path, vocab_path, input_strings, device):
    char_dim = 20  # 每个字的维度
    sentence_length = 6  # 样本文本长度
    ...
    x = torch.LongTensor(x).to(device=device)
    model.eval()   #测试模式
    with torch.no_grad():  #不计算梯度
        result = model.forward(torch.LongTensor(x))  #模型预测
    y_pred_classes = torch.argmax(result, dim=1)
    for i, input_string in enumerate(input_strings):
        position = y_pred_classes[i].item()
        if position < sentence_length:
            print("输入：%s, 预测位置：%d" % (input_string, position))
        else:
            print("输入：%s, 预测位置：不在字符串中" % input_string)
...
if __name__ == "__main__":
    ...
    predict("model.pth", "vocab.json", test_strings, torch.device("cuda" if torch.cuda.is_available() else "cpu"))
