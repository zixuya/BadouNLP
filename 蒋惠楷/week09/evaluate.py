...
class Evaluator:
    def __init__(self, config, model, logger):
        ...
        self.results_file = "./model_results.xlsx"
    ...
    def show_stats(self, epoch):
        F1_scores = []
        stats_data = {}  # 用于存储当前轮次的统计结果
        stats_data["Model"] = self.config["model_type"]
        for key in ["PERSON", "LOCATION", "TIME", "ORGANIZATION"]:
            # 正确率 = 识别出的正确实体数 / 识别出的实体数
            # 召回率 = 识别出的正确实体数 / 样本的实体数
            ...
            stats_data[f"{key}_Precision"] = precision
            stats_data[f"{key}_Recall"] = recall
            stats_data[f"{key}_F1"] = F1

        macro_f1 = np.mean(F1_scores)
        self.logger.info("Macro-F1: %f" % macro_f1)
        stats_data["Macro-F1"] = macro_f1
        ...
        self.logger.info("--------------------")
        stats_data["Micro-F1"] = micro_f1

        # 将最后一轮结果保存到Excel
        if epoch == self.config["epoch"]:
            self.save_to_excel(stats_data)
        return
    
    def save_to_excel(self, stats_data):
        # 将当前轮次的结果转换为DataFrame
        df_current = pd.DataFrame([stats_data])

        # 检查文件是否存在
        if Path(self.results_file).exists():
            # 如果文件存在，读取现有数据并追加新数据
            df_existing = pd.read_excel(self.results_file)
        ...
        # 保存到Excel
        df_updated.to_excel(self.results_file, index=False)
        self.logger.info(f"测试结果已保存至 {self.results_file}")
