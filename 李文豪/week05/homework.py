...
def main():
    ...
    print(len(vector_label_dict))
    print(vector_label_dict.keys())
    print("==========================")
    print(kmeans.cluster_centers_)
    print("==========================")
    for  label ,vector_s in vector_label_dict.items():
        # print(vector_s)
        centen = kmeans.cluster_centers_[label]
        distances = []
        for vector in vector_s:
            distance = np.sqrt(np.sum((vector - centen) ** 2))
            distances.append(distance)
        distances.sort()
        print("便签为：%d的类内距离排序如下")
        print(distances)
        # print(vector_ss)
