...
class DataGenerator:
    ...
    def random_train_sample(self):
        ...
        k = random.choice(standard_question_index)
        #如果选取到的标准问下不足两个问题，则无法选取，所以重新随机一次
        if len(self.knwb[k]) < 2:
            return self.random_train_sample()
        else:
            a, p = random.sample(self.knwb[k], 2)
            j = random.choice(standard_question_index)
            while j == k:
                j = random.choice(standard_question_index)
            n = random.sample(self.knwb[j], 1)[0]
            return [a, p, n]
        #随机负样本
        # else:
