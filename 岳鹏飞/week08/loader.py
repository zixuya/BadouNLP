...
class DataGenerator:
    ...
    #正样本从随机一个标准问题中随机选取两个
    #如果选用triplet作为loss函数，需要在一个类别的样本中选择两个样本分别作为锚点和正样本，另外一个类别中选择一个样本作为负样本
    def random_train_sample(self):
        ...
        #     return [s1, s2, torch.LongTensor([-1])]
        s1, s2 = random.sample(standard_question_index, 2) # 随机选择两个类别
        if len(self.knwb[s1]) < 2:
            return self.random_train_sample()
        a, p = random.sample(self.knwb[s1], 2) # 在同一个类别中选择锚点样本，和正样本
        n = random.choice(self.knwb[s2]) # 在另一个类别中随机挑选一个样本作为负样本
        return [a, p, n]
