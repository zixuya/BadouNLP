...
class DialogueSystem:
    ...
    def load(self):
        self.nodes_info = {}
        self.load_scenario("scenario-买衣服1.json")
    ...
    def dpo(self, memory):
        ...
        if memory["hit_node"] == "scenario-买衣服1node5":
            ...
            if memory["require_slot"] is None:
                ...
                memory["policy"] = "ask"
                memory["available_nodes"] = [memory["hit_node"], "scenario-买衣服1node5"]  # 停留在当前节点，直到槽位填满
        return memory

    def nlg(self, memory):
        # 根据policy生成回复,反问或回复
        if memory["policy"] == "repeat":
            memory["response"] = self.nodes_info[memory["hit_node"]]["response"] + memory["past"]["response"]
...
if __name__ == '__main__':
    ds = DialogueSystem()
    print(ds.slot_to_qv)
    memory = {"available_nodes": ["scenario-买衣服1node1"], "past": {}}  # 默认初始记忆为空
