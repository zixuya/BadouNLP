...
def main():
    model = load_word2vec_model(r"D:\桌面\资料\第五周\week5 词向量及文本向量\model.w2v") #加载词向量模型
    sentences = load_sentence(r"D:\桌面\资料\第五周\week5 词向量及文本向量\titles.txt")  #加载所有标题
    ...
    for vector_index, label in enumerate(kmeans.labels_):
        vector = vectors[vector_index]   #某句话的向量 0,1,2...1796
        center = kmeans.cluster_centers_[label]  #对应的类别中心向量 label:0...41
    ...
    density_order = sorted(density_dict.items(), key=lambda x:x[1], reverse=True)  #按照平均距离排序，向量夹角余弦值越接近1，距离越小
         #density_dict.items()返回的是类似[(1, 2), (2, 3), (3, 4)]的元组序列，因为是对每个元组的第二个值进行排序，故添加key=lambda x:x[1]；降序，故reverse=True
    #按照余弦距离顺序输出
