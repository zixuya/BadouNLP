...
class DataGenerator:
    ...
    def random_train_sample(self):
        ...
        # 从锚点样本所在类别中选取正样本
        positive_candidates = [s for s in self.knwb[anchor_class] if torch.all(torch.ne(s, anchor))]
        if len(positive_candidates) == 0:
            # 如果没有其他正样本，重新采样
            return self.random_train_sample()
        positive = random.choice(positive_candidates)
        # 选取负样本所在的类别，确保与锚点样本所在类别不同
        negative_class_candidates = [k for k in standard_question_index if k != anchor_class]
...
if __name__ == "__main__":
    ...
    dg = DataGenerator("D:\\桌面\\资料\\第八周\\week8 文本匹配问题\\data\\train.json", Config)
    print(dg[1])
