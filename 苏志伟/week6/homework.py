...
bert = BertModel.from_pretrained(r"D:\BADOU\第六周 语言模型\bert-base-chinese\bert-base-chinese", return_dict=False)
...
class DiyBert:
    ...
    def cal_para(self):
        num_para_embd = 0
        num_para_all_transformer_layer = 0
        num_para_pool = 0
        num_para_word_embeddings 				= state_dict["embeddings.word_embeddings.weight"].numpy().size
        num_para_position_embeddings 			= state_dict["embeddings.position_embeddings.weight"].numpy().size
        num_para_token_type_embeddings 			= state_dict["embeddings.token_type_embeddings.weight"].numpy().size
        num_para_embeddings_layer_norm_weight 	= state_dict["embeddings.LayerNorm.weight"].numpy().size
        num_para_embeddings_layer_norm_bias 	= state_dict["embeddings.LayerNorm.bias"].numpy().size
        num_para_embd = num_para_word_embeddings + num_para_position_embeddings + num_para_token_type_embeddings + num_para_embeddings_layer_norm_weight + num_para_embeddings_layer_norm_bias
        for i in range(self.num_layers):
            num_para_q_w = state_dict["encoder.layer.%d.attention.self.query.weight" % i].numpy().size
            num_para_q_b = state_dict["encoder.layer.%d.attention.self.query.bias" % i].numpy().size
            num_para_k_w = state_dict["encoder.layer.%d.attention.self.key.weight" % i].numpy().size
            num_para_k_b = state_dict["encoder.layer.%d.attention.self.key.bias" % i].numpy().size
            num_para_v_w = state_dict["encoder.layer.%d.attention.self.value.weight" % i].numpy().size
            num_para_v_b = state_dict["encoder.layer.%d.attention.self.value.bias" % i].numpy().size
            num_para_attention_output_weight = state_dict["encoder.layer.%d.attention.output.dense.weight" % i].numpy().size
            num_para_attention_output_bias = state_dict["encoder.layer.%d.attention.output.dense.bias" % i].numpy().size
            num_para_attention_layer_norm_w = state_dict["encoder.layer.%d.attention.output.LayerNorm.weight" % i].numpy().size
            num_para_attention_layer_norm_b = state_dict["encoder.layer.%d.attention.output.LayerNorm.bias" % i].numpy().size
            num_para_intermediate_weight = state_dict["encoder.layer.%d.intermediate.dense.weight" % i].numpy().size
            num_para_intermediate_bias = state_dict["encoder.layer.%d.intermediate.dense.bias" % i].numpy().size
            num_para_output_weight = state_dict["encoder.layer.%d.output.dense.weight" % i].numpy().size
            num_para_output_bias = state_dict["encoder.layer.%d.output.dense.bias" % i].numpy().size
            num_para_ff_layer_norm_w = state_dict["encoder.layer.%d.output.LayerNorm.weight" % i].numpy().size
            num_para_ff_layer_norm_b = state_dict["encoder.layer.%d.output.LayerNorm.bias" % i].numpy().size
			
            num_para_all_transformer_layer += (num_para_q_w + num_para_q_b + num_para_k_w + num_para_k_b + num_para_v_w + num_para_v_b + num_para_attention_output_weight + num_para_attention_output_bias + num_para_attention_layer_norm_w + num_para_attention_layer_norm_b + num_para_intermediate_weight + num_para_intermediate_bias + num_para_output_weight + num_para_output_bias + num_para_ff_layer_norm_w + num_para_ff_layer_norm_b )
        num_para_pooler_dense_weight = state_dict["pooler.dense.weight"].numpy().size
        num_para_pooler_dense_bias = state_dict["pooler.dense.bias"].numpy().size
        num_para_pool += (num_para_pooler_dense_weight + num_para_pooler_dense_bias)
        return num_para_embd + num_para_all_transformer_layer + num_para_pool 
