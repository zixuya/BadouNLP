...
class DailogueSystem:
    ...
    def intent_recognition(self, memory):
        ...
        max_score_node = None
        max_node_name = None
        re_query_node = {"intent":["我要重听", "我没听清楚，能重新说一遍吗", "没听清，能再说一遍吗"], "re_query":True}
        for node_name in memory["available_nodes"]:
            ...
            if score > max_score:
                max_score = score
                max_score_node = node_info
                max_node_name = node_name
        score = self.get_node_score(memory["query"], re_query_node)
        if score > max_score:
                max_score = score
                max_score_node = re_query_node
        if max_score_node is not None:
            is_re_query = max_score_node.get("re_query", False)
            if is_re_query:
                memory["re_query"] = True
            else:
                memory["hit_node"] = max_node_name
                memory["re_query"] = False
        return memory
