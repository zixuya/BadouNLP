...
bert = BertModel.from_pretrained(r"D:\AIClass\第六周 语言模型\bert-base-chinese", return_dict=False)
...
# 遍历state_dict中的参数，计算数量
for param_tensor in state_dict.values():
    # param_tensor.size()返回一个元组，表示参数的各个维度
    # 我们使用-1来指定我们想要计算所有维度的乘积
    total_params += np.prod(param_tensor.size())
print(f"Bert参数量: {total_params}")
