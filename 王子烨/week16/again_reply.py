...
class DailogueSystem:
    ...
    def intent_recognition(self, memory):
        ...
        for node_name in memory["available_nodes"]:
            ...
            memory, score = self.get_node_score(memory, memory["query"], node_info)
            if score > max_score:
                max_score = score
                memory["hit_node"] = node_name
        return memory

    def get_node_score(self, memory, query, node_info):
        # 跟node中的intent算分
        ...
        for intent in intent_list:
            ...
            if score < self.sentence_match_score(query, intent):
                score = self.sentence_match_score(query, intent)
