...
class Evaluator:
    ...
    def eval(self, epoch):
        ...
        for index, batch_data in enumerate(self.valid_data):
            ...
            with torch.no_grad():
                #  similarity_score:余弦距离
                similarity_score = self.model(input_id1, input_id2)  # 不输入labels，使用模型当前参数进行预测
                # 将 similarity_score 中小于 0.3 的值设置为 1，大于等于 0.3 的值设置为 -1
                predictions = torch.where(similarity_score < 0.3, torch.tensor(1.0), torch.tensor(-1.0))
            self.write_stats(predictions, labels)
        self.show_stats()
        return
    ...
    #     return
    def write_stats(self, prediction, labels,threshold=0.5):
