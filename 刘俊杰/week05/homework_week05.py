...
def main():
    ...
    for vector_index, label in enumerate(kmeans.labels_):
        ...
        cluster_centers = kmeans.cluster_centers_[label]  # 对应的类别中心向量
        distance = cosine_distance(vector, cluster_centers)  # 计算欧式距离
    ...
    density_order = sorted(density_dict.items(), key=lambda x: x[1], reverse=True)  # 按照平均距离排序
