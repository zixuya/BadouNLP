...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        x = self.embedding(x)           #batchsize seq_len vector_dim
        rnn_out, hidden = self.rnn(x)   #batchsize seq_len vector_dim
        x = rnn_out[:, -1, :]  #batchsize vector_dim

        #接线性层做分类
        y_pred = self.classify(x)         #batchsize sentence_length + 1
...
#随机生成一个样本
def build_sample(vocab, sentence_length, target_char='a'):
...
if __name__ == "__main__":
    main()
    test_strings = ["kijdefh", "gijkfkk", "gkijdfk", "kijhdef"]
