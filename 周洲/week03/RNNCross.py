...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab):
        ...
        self.RNNlayer = nn.RNN(sentence_length, vector_dim, bias=False, batch_first=True)
...
def build_sample(vocab, sentence_length, target_char):
    ...
    # 指定哪些字出现时为正样本
    if set(target_char) & set(x):
        target_indices = [i for i, char in enumerate(x) if char == target_char]
        y = target_indices[0] + 1
    # 指定字都未出现，则为负样本
    else:
        y = 0
...
def predict(model_path, vocab_path, input_strings):
    ...
    for i, input_string in enumerate(input_strings):
        max_value, max_index = torch.max(result[i], dim=0)
        if max_index == 0:
            print("输入：%s, 序列中不存在该字符" % input_string)
        else:
            print("输入：%s, 该字符最可能在第%d个, 其概率为：%f" % (input_string, max_index, max_value))  # 打印结果


if __name__ == "__main__":
    target_char = "我"
    main(target_char)
    print("当前特定字符为:", target_char)
    test_strings = ["fn我fee", "wz你dfg", "rqwdeg", "n我kwww", "eft他ll"]
