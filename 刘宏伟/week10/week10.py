...
class LanguageModel(nn.Module):
    def __init__(self, input_dim, vocab):
        ...
        self.bert = BertModel.from_pretrained(r'E:\nlp\bert-base-chinese', return_dict=False)
    ...
    def forward(self, x, y=None, mask=None):
        ...
        if mask is None:
            ...
            if torch.cuda.is_available():
                x = x.cuda()
                mask = mask.cuda()
            x, _ = self.bert(input_ids=x, attention_mask=mask.cuda())
...
def train(corpus_path, save_weight=True):
    ...
    vocab = build_vocab("lstm语言模型生成文本/vocab.txt")  # 建立字表
    ...
    for epoch in range(epoch_num):
        ...
        for batch in range(int(train_sample / batch_size)):
            ...
            # mask = torch.ones((batch_size, window_size), dtype=torch.long)
