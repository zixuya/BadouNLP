    ...
    def load(self):
        ...
        with open(self.path, encoding="utf8") as f:
            ...
            for line in f:
                ...
                if random.random() > 0.2:  # 用20%的数据进行测试
                    self.data.append([torch.LongTensor(input_id), torch.LongTensor([label])])
                else:
                    self.test.append([torch.LongTensor(input_id), torch.LongTensor([label])])
        return
...
def load_data(data_path, config, shuffle=True):
    ...
    test = DataLoader(dg.test, batch_size=config["batch_size"], shuffle=shuffle)
    return dl, test
