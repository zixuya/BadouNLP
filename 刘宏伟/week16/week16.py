...
class DailogueSystem:
    ...
    def load(self):
        self.nodes_info = {"repeat": {"id": "repeat", "intent": ["重复", "再说一遍"]}}
    ...
    def generate_response(self, query, memory):
        ...
        if memory["hit_node"] == "repeat":
            memory["response"] = memory["previous_response"]
            return memory
        # print(memory)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # 第一次回复后，将回复加入记忆
        memory["previous_response"] = memory["response"]
        memory["available_nodes"].append("repeat")
