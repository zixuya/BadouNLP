...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab):
        ...
        self.classify = nn.Linear(vector_dim, sentence_length + 1)  # 线性层，输出维度为句子长度+1（包括不在字符串中的情况）
    ...
    def forward(self, x, y=None):
        ...
        x = x[:, -1, :]  # 取最后一个时间步的输出
        y_pred = self.classify(x)  # (batch_size, vector_dim) -> (batch_size, sen_len + 1)
        if y is not None:
...
# 如果特定字符出现，则标记其位置，否则标记为sentence_length
...
def predict(model_path, vocab_path, input_strings):
    ...
    for i, input_string in enumerate(input_strings):
        print("输入：%s, 预测位置：%d" % (input_string, result[i])) # 打印结果
