...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab):
        ...
        self.classify = nn.Linear(vector_dim, sentence_length)  # 线性层，输出每个位置的概率
...
def build_sample(vocab, sentence_length):
    ...
    # 指定哪些字出现时为正样本
    target_char = random.choice(["你", "我", "他"])
    target_index = random.randint(0, sentence_length - 1)
    x[target_index] = target_char
    y = target_index  # y是类别索引
...
def predict(model_path, vocab_path, input_strings, sentence_length):
    ...
    model.eval()  # 测试模式
    with torch.no_grad():  # 不计算梯程
        result = model.forward(torch.LongTensor(x))  # 模型预测
    for i, input_string in enumerate(input_strings):
        print("输入：%s, 预测位置：%d" % (input_string, result[i].argmax().item()))  # 打印结果

if __name__ == "__main__":
    main()
    test_strings = ["fnvfee", "wz你dfg", "rqwdeg", "n我kwww"]
    predict("model.pth", "vocab.json", test_strings, 6)
