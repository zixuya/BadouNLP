...
class Evaluator:
    ...
    def eval(self, epoch):
        ...
        for index, batch_data in enumerate(self.valid_data):
            ...
            input_id, attention_mask, labels = batch_data  # 解包三个值
            with torch.no_grad():
                pred_results = self.model(input_id, attention_mask=attention_mask)  # 传递 attention_mask
    ...
    def write_stats(self, labels, pred_results, sentences):
        ...
        for true_label, pred_label, sentence in zip(labels, pred_results, sentences):
            ...
            # 跳过[CLS]和[SEP]标记符
            true_label = [label for label in true_label if label != -500]
            pred_label = [label for label in pred_label if label != -500]
    ...
    def decode(self, sentence, labels):
        labels = [label for label in labels if label != -500]  # 跳过-500的标签
