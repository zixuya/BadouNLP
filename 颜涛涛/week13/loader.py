    ...
    def load(self):
        ...
        with open(self.path, encoding="utf8") as f:
            ...
            for segment in segments:
                ...
                # 文本转化成向量 并且处理文本长度问题 长度不够则补0
                # input_ids = self.encode_sentence(sentenece)
                if self.config['model_type'] == 'bert':
                    input_ids = self.tokenizers.encode(sentenece, max_length=self.config['max_length'],
                                                       pad_to_max_length=True,add_special_tokens=False)
                #     处理cls  sep保证长度对齐
                #     input_ids = [0 if item == 102 else item for item in input_ids[1:100] + [0]]
                else:
                    input_ids = self.encode_sentence(sentenece)
                    # label和文本长度对齐，并且补-1
