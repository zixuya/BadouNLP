...
class DataGenerator:
    ...
    def random_train_sample(self):
        ...
        #     return [s1, s2, torch.LongTensor([-1])]
        p_label, n_label = random.sample(standard_question_index, 2)
        # 如果正样本少于两个
        if len(self.knwb[p_label]) < 2:
            return self.random_train_sample()
        else:
            # 一个原点一个正样本
            a_sentence, p_sentence = random.sample(self.knwb[p_label], 2)
            # 负样本
            # n_sentence = random.sample(self.knwb[n_label], 1)
            n_sentence = random.choice(self.knwb[n_label])
