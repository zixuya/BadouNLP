...
class LanguageModel(nn.Module):
    ...
    def forward(self, x, y=None, question_len=None):
        if y is not None and question_len is not None:
...
def train(corpus_path, save_weight=True):
    ...
    train_data_path = r'E:\badou1117\八斗精品课1117\第十周 文本生成\week10 文本生成问题\homework\test.json'
    tokenizer = BertTokenizer.from_pretrained(pretrain_model_path)

    train_data = load_data(train_data_path, batch_size, pretrain_model_path)  # 加载语料
    ...
    for epoch in range(epoch_num):
        ...
        for index, batch_data in enumerate(train_data):
            x, y, question_len = batch_data
            if torch.cuda.is_available():
                x, y = x.cuda(), y.cuda()
            optim.zero_grad()  # 梯度归零
            loss = model(x, y, question_len)  # 计算loss
            loss.backward()  # 计算梯度
            optim.step()  # 更新权重
            watch_loss.append(loss.item())
        print("=========\n第%d轮平均loss:%f" % (epoch + 1, np.mean(watch_loss)))
        print(generate_sentence("张亮厨师", model, tokenizer))
        print(generate_sentence("污染企业", model, tokenizer))
    if not save_weight:
        return
    else:
