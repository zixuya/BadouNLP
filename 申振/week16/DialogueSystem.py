...
class DailogueSystem:
    ...
    def load_scenario(self, scenario_file):
        with open(self.load_dir + scenario_file, "r", encoding="utf-8") as f:
    ...
    def load_slot_template(self, slot_template_file):
        self.slot_template = pd.read_excel(self.load_dir + slot_template_file)
        #slot	query	values
        self.slot_to_qv = {}
    ...
    def intent_recognition(self, memory):
        ...
        # 先判断是否是重问
        pardon_values = "没听清楚|麻烦在说一遍"
        if re.search(pardon_values, memory["query"]):
            print("pardon",memory["query"])
            return memory
        max_score = -1
