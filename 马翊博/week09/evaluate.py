...
class Evaluator:
    ...
    def eval(self, epoch):
        ...
        for index, batch_data in enumerate(self.valid_data):
            input_ids, attention_masks, labels, sentences = batch_data  # 输入变化时这里需要修改，比如多输入，多输出的情况
            if torch.cuda.is_available():
                input_ids = input_ids.cuda()
                attention_masks = attention_masks.cuda()
                labels = labels.cuda()
            with torch.no_grad():
                pred_results = self.model(input_ids, attention_masks)  # 不输入labels，使用模型当前参数进行预测
