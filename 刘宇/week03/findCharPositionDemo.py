...
def build_sample(vocab, sentence_length):
    ...
    # 指定哪些字出现时为正样本
    target_chars = set("你我他")
    positions = [i for i, char in enumerate(x) if char in target_chars]
    if positions:
        y = positions[0]  # 取第一个出现的目标字符的位置
    else:
        y = sentence_length  # 如果没有目标字符，设置为句子长度
...
def predict(model_path, vocab_path, input_strings):
    ...
    for i, input_string in enumerate(input_strings):
        position = predicted_classes[i].item()
        if position < sentence_length:
            print("输入：%s, 预测位置：%d, 概率值：%f" % (input_string, position, result[i][position]))
        else:
            print("输入：%s, 预测位置：未找到, 概率值：%f" % (input_string, result[i][position]))
