...
def main():
    ...
    top_k = 10 # 随便写一个前十小的平均距离
    distance_array = np.zeros(len(centers))
    ...
    # 计算每个类别的平均距离
    for label, vector_list in vector_label_dict.items():
        center = centers[label]
        distance_array[label] = np.linalg.norm(np.array(vector_list) - center, axis=1).sum() # k, 128 - 128 -> k, 128 -> k -> scalar

    print(f"前{top_k}个类别")
    top_k_labels = np.argsort(distance_array)[:top_k]
    print(top_k_labels)
    print("---------")
