...
class LanguageModel(nn.Module):
    def __init__(self, vocab):
        ...
        self.bert = BertModel.from_pretrained("/six-lm/bert-base-chinese")
...
def build_sample(vocab, window_size, corpus):
    ...
    y = [vocab.get(word, vocab["[UNK]"]) for word in target] # BERT用的UNK括号跟之前代码不一样
    return x, y
...
def generate_sentence(openings, model, vocab, window_size, device):
    ...
    with torch.no_grad():
        ...
        while pred_char != "\n" and len(openings) <= 30:
            ...
            x = torch.LongTensor([x]) # 这里隐式弄成了Batch
            x = x.to(device)
            y = model(x)[0][-1]
...
def train(corpus_path, save_weight=True):
    ...
    # 这么做相当于使用了Bert的字表，但没有用BERT的编码器
    vocab = build_vocab("/six-lm/bert-base-chinese/vocab.txt")
    ...
    for epoch in range(epoch_num):
        ...
        for batch in range(int(train_sample / batch_size)):
            ...
            causal_mask = torch.tril(torch.ones(x.shape[1], x.shape[1])).unsqueeze(0)
            causal_mask = causal_mask.expand(x.shape[0], -1, -1)
            loss = model(x, y, causal_mask.to(device))   #计算loss
            loss.backward()      #计算梯度
            optim.step()         #更新权重
