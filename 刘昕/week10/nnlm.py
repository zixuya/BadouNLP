...
class LanguageModel(nn.Module):
    ...
    def forward(self, x, y=None, attention_mask=None):
        if attention_mask is None:
            attention_mask = torch.tril(torch.ones(x.shape[-1], x.shape[-1]))
...
def build_sample(window_size, corpus):
    ...
    window = split_numbers(window)
    target = split_numbers(target)
    # print(window, target)
    ...
    with torch.no_grad():
        ...
        while pred_char != "\n" and len(openings) <= 30:
            ...
            if torch.cuda.is_available():
                x = x.cuda()
            y = model(x, attention_mask=mask_attention)[0][-2]
...
def calc_perplexity(sentence, model, vocab, window_size):
    ...
    with torch.no_grad():
        for i in range(1, len(sentence)):
            ...
            if torch.cuda.is_available():
                x = x.cuda()
            pred_prob_distribute = model(x)[0][-2]
...
def train(corpus_path, save_weight=True):
    ...
    if torch.cuda.is_available():
        model = model.cuda()
    optim = torch.optim.Adam(model.parameters(), lr=5e-4)   #建立优化器
    print("文本词表模型加载完毕，开始训练")
    for epoch in range(epoch_num):
        model.train()
        watch_loss = []
        for batch in range(int(train_sample / batch_size)):
            x, y, attention_masks = build_dataset(batch_size, window_size, corpus) #构建一组训练样本
            if torch.cuda.is_available():
                x, y = x.cuda(), y.cuda()
