...
class LanguageModel(nn.Module):
    ...
    def __init__(self, pretrain_model_path):
        ...
        # ignore_index=-1：指定在计算损失时忽略标签值为 -1 的样本
        # 这在处理填充或无效标签时非常有用。
...
def build_dataset(tokenizer, corpus, max_length, batch_size):
    ...
    for i, (prompt, answer) in enumerate(corpus):
        prompt_encode = tokenizer.encode(
            prompt, add_special_tokens=False)  # length: len_s1
        answer_encode = tokenizer.encode(
            answer, add_special_tokens=False)  # length: len_s2
        # 拼接成一个完整的输入序列
        x = [tokenizer.cls_token_id] + prompt_encode + [
            tokenizer.sep_token_id
        ] + answer_encode + [tokenizer.sep_token_id]
        # answer 部分的标签为其对应的词元索引，最后一个 [SEP] 词元的标签也为其索引，其余位置为 -1。
        ...
        mask = create_mask(
            len(prompt_encode),
            len(answer_encode))  # shape: (len_s1 + len_s2, len_s1 + len_s2)
        #padding
    ...
    # 使用 DataLoader 对数据集进行批量处理，设置批次大小为 batch_size，并在每个训练周期开始时随机打乱数据。num_workers=0 表示不使用多线程加载数据。
...
def pad_mask(tensor, target_shape):
    ...
    # 将原始张量对应的部分填充到全零张量中
    result[h_start:h_end,
           w_start:w_end] = tensor[:h_end - h_start, :w_end - w_start]
    return result
...
def generate_sentence(openings, model, tokenizer):
    model.eval()
    # openings：字符串类型的参数，生成句子的起始文本。
    # model：已经训练好的语言模型，预测下一个词元。
    # tokenizer：文本和词元索引转换
    # 分词器将输入的开头文本 openings 编码为词元索引列表。
    ...
    with torch.no_grad():
        ...
        while len(openings) <= 50:
            ...
            if torch.cuda.is_available():
                x = x.cuda()
            # [0] 取批次中的第一个样本，[-1] 取该样本的最后一个词元的预测结果。
            y = model(x)[0][-1]
...
def main(corpus_path, save_weight=True):
    ...
    pretrain_model_path = r'C:\Users\ada\Desktop\学习\八斗AI课\第六周 语言模型\bert-base-chinese'
