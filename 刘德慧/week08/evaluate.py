...
class Evaluator:
    ...
    def __init__(self, config, model, logger):
        ...
        self.model = model
        self.logger = logger
        self.valid_data = load_data(config["valid_data_path"],
                                    config,
                                    shuffle=False)
    ...
    def knwb_to_vector(self):
        ...
        for standard_question_index, question_ids in self.train_data.dataset.knwb.items(
            ...
            for question_id in question_ids:
                ...
                self.question_index_to_standard_question_index[len(
                    self.question_ids)] = standard_question_index
                self.question_ids.append(question_id)
        with torch.no_grad():
            question_matrixs = torch.stack(self.question_ids,
                                           dim=0)  #shape = [n, max_length]
            if torch.cuda.is_available():
                question_matrixs = question_matrixs.cuda()
            self.knwb_vectors = self.model(question_matrixs)  # 前向传播
            # 'NoneType' object has no attribute 'norm'？？？
            self.knwb_vectors = torch.nn.functional.normalize(
                self.knwb_vectors, dim=-1)
        return
    ...
    def eval(self, epoch):
        ...
        for index, batch_data in enumerate(self.valid_data):
            ...
            with torch.no_grad():
                test_question_vectors = self.model(
                    input_id)  #不输入labels，使用模型当前参数进行预测
    ...
    def write_stats(self, test_question_vectors, labels):
        ...
        for test_question_vector, label in zip(test_question_vectors, labels):
            ...
            hit_index = self.question_index_to_standard_question_index[
                hit_index]  #转化成标准问编号
