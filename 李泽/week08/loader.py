...
class DataGenerator:
    ...
    def __getitem__(self, index):
        if self.data_type == "train":
            ...
            input_id, label_index = self.data[index]
            return input_id, label_index
    ...
    def random_train_sample(self):
        ...
        #随机负样本
            ...
            # 随机选择一个不同的标准问题作为正样本
            p2 = random.choice([q for q in standard_question_index if q != n])
            s3 = random.choice(self.knwb[p2])
            return [s1, s3, s2, torch.LongTensor([-1])]
