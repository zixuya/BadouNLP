...
基于pytorch的网络编写
实现一个网络完成一个简单nlp任务
判断文本中是否有‘你’字符出现，字符在哪就是第几类，字符未出现则赋值为(句子最大长度)
rnn lstm 实现 多分类任务
...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab,num_classes):
        ...
        self.classify = nn.Linear(vector_dim, num_classes)     #线性层.多分类任务,句子长度+1维度（最后一类是为空的一位）
        # self.layer = nn.RNN(input_size, hidden_size, bias=False, batch_first=True)
        # self.activation = torch.sigmoid    # softmax 归一化函数
        # self.activation = torch.softmax    # softmax 归一化函数
...
# 如果字集中包含"你"，则该分类为 ‘你’出现的下标，否则分类为句子长度
...
def predict(model_path, vocab_path, input_strings):
    ...
    model = build_model(vocab, char_dim, sentence_length,sentence_length+1)     #建立模型
    ...
    for i, input_string in enumerate(input_strings):
        print("输入：%s, ‘你’出现位置的下标（6表示没有出现的默认值）：%d" % (input_string, round(float(result[i])))) #打印结果
...
if __name__ == "__main__":
    # vocab = build_vocab()
    # x, y = build_dataset(10, vocab, 5)   #建立200个用于测试的样本
    # print(x, y)
    main()
    test_strings = ["fnvfee", "wz你dfg", "rqwdeg", "n你kwww"]
