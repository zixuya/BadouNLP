...
class SiameseNetwork(nn.Module):
    ...
    def cosine_distance(self, tensor1, tensor2):
        tensor1 = tensor1.float()  # 确保输入是浮点类型
        tensor2 = tensor2.float()  # 确保输入是浮点类型
    ...
    def forward(self, sentence1, sentence2=None, target=None):
        ...
        if sentence2 is not None:
            ...
            if target is not None:
                # 更改成 triplet loss使用的张量 ,余弦loss可忽略 target
                return self.loss(vector1, vector2, self.sentence_encoder(target.squeeze()))
...
if __name__ == "__main__":
    ...
    # 更改成 triplet loss使用的张量
    l = torch.LongTensor([[1,2,3,0], [3,2,3,0]])
    y = model(s1, s2, l)
    print(y)
