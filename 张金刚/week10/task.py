...
class LanguageModel(nn.Module):
    ...
    def forward(self, x, y=None):
        if use_bert:  # bert返回的结果是 (sequence_output, pooler_output)
...
def build_sample(vocab, window_size, corpus):
    ...
    if use_bert:
        ...
        x = tokenizer.encode(window, add_special_tokens=False,truncation=True,max_length=window_size, pad_to_max_length=True)
        y = tokenizer.encode(target, add_special_tokens=False,truncation=True,max_length=window_size, pad_to_max_length=True)
    else:
