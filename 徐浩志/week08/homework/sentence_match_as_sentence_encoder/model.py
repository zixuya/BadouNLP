...
class SiameseNetwork(nn.Module):
    ...
    def forward(self, sentence1, sentence2=None, target=None):
        ...
        if sentence2 is not None:
            ...
            vector3 = self.sentence_encoder(target)
            #如果有标签，则计算loss
            if target is not None:
                return self.loss(vector1, vector2, vector3)    # output = triplet_loss(anchor, positive, negative)
            #如果无标签，计算余弦距离
            else:
...
if __name__ == "__main__":
    ...
    negative = torch.LongTensor([[1,2,3,5],[3,2,3,4]])
