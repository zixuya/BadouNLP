...
class Evaluator:
    def __init__(self, config, model, logger):
        ...
        self.valid_data = load_data(config["valid_data_path"], config, shuffle=False)#不打乱数据集


    def eval(self, epoch):
        self.logger.info("开始测试第%d轮模型效果：" % epoch)#日志记录
        self.stats_dict = {"LOCATION": defaultdict(int),
                           "TIME": defaultdict(int),
                           "PERSON": defaultdict(int),
                           "ORGANIZATION": defaultdict(int)}#统计字典
    ...
    def show_stats(self):
        ...
        self.logger.info("Micro-F1 %f" % micro_f1)#micro-F1,使用 self.logger 来记录测试开始的信息。
        self.logger.info("--------------------")
        return
...
def decode(self, sentence, labels):
    ...
    for location in re.finditer("(04+)", labels):
        e = location.span()
        results["LOCATION"].append(sentence[:e])
    for location in re.finditer("(15+)", labels):
        e = location.span()
        results["ORGANIZATION"].append(sentence[:e])
    for location in re.finditer("(26+)", labels):
        e = location.span()
        results["PERSON"].append(sentence[:e])
    for location in re.finditer("(37+)", labels):
        e = location.span()
        results["TIME"].append(sentence[:e])
    return results