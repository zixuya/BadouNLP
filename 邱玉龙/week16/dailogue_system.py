...
class DailogueSystem:
    ...
    def intent_recognition(self, memory):
        ...
        # 回退意图识别
        self.rollback_check(memory, max_score)

        return memory

    def rollback_check(self, memory, max_score):
        rollback_node = self.nodes_info["scenario-买衣服node5"]
        memory["rollback"] = False
        if self.get_node_score(memory["query"], rollback_node) > max_score:
            if memory["pre_node"]:

                pre_node_info = memory["pre_node"][-1]
                if pre_node_info.find("|") != -1:
                    pre_node_infos = pre_node_info.split("|")
                    node_name = pre_node_infos[0]
                    slot = pre_node_infos[1]
                else:
                    node_name = pre_node_info
                    slot = None
                memory["hit_node"] = node_name
                memory["require_slot"] = slot
                memory["pre_node"].remove(pre_node_info)
                memory["rollback"] = True
    ...
    def slot_filling(self, memory):
        ...
        for slot in slot_list:
            ...
            if re.search(slot_values, memory["query"]):
                ...
                #记录当前节点和槽位
                memory["pre_node"].append(memory["hit_node"] + f'|{slot}')
        return memory
...
if __name__ == '__main__':
    ...
    print(ds.slot_to_qv)
    # pre_node  回退节点存储
    memory = {"available_nodes":["scenario-买衣服node1"], "pre_node": []}  #默认初始记忆为空
    while True:
