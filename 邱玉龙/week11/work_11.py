        ...
        if y is not None:
            ...
            for i in range(batch_size):
                for j in range(seq_len):
                    # 目标序列起始位置（即 <sep> 后面的部分）才需要计算损失
                    if x[i, j] == self.tokenizer.encode('<sep>')[0]:
                        mask[i, j + 1:, j + 1:] = 0  # 目标序列之后的位置不会影响损失
                    elif x[i, j] == self.tokenizer.encode('<eos>')[0]:
                        mask[i, j, :] = 0  # EOS 之后不参与计算
...
def build_sample(tokenizer, window_size, corpus):
    ...
    # 为输入和输出分别添加特殊符号
    input_text = window + '<sep>'
    output_text = target + '<eos>'
    x = tokenizer.encode(input_text, add_special_tokens=False, padding='max_length', truncation=True, max_length=10)   #将字转换成序号
    y = tokenizer.encode(output_text, add_special_tokens=False, padding='max_length', truncation=True, max_length=10)
