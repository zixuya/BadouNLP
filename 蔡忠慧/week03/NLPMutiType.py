...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab,hidden_size):
        ...
        self.loss = nn.CrossEntropyLoss()    #loss函数交叉熵损失

    #当输入真实标签，返回loss值；无真实标签，返回预测值
    def forward(self, x, y=None):
        x = self.embedding(x)                      #(batch_size, sen_len, vector_dim)
        out, _ = self.rnn(x)                       #(batch_size, sen_len, hidden_size)
        x = out[:,-1,:]                            #(batch_size, hidden_size)   rnn最后一步会包含前面元素的信息
        x = self.classify(x)                       #(batch_size, hidden_size) -> (batch_size, sentence_length+1) 3*5 5*7 -> 3*7
        x = self.dp(x)
        y_pred = self.activation(x)                #(batch_size, 7) -> (batch_size, 7)
...
#abc -> [1,2,3]
def build_vocab():
    chars = "你我他defghijklmnopqrstuvwxyz里打"  #字符集
    vocab = {"pad":0}
...
def build_dataset(sample_length, vocab, sentence_length):
    ...
    # print('dataset_y', len(dataset_y), dataset_y[0])
...
def predict(model_path, vocab_path, input_strings):
    ...
    model = build_model(vocab, char_dim, sentence_length,hidden_size=128)     #建立模型
    ...
    for i, input_string in enumerate(input_strings):
        maxIndex = torch.argmax(result[i])
        print("输入：%s, 预测类别：%s, 概率值：%f" % (input_string, torch.argmax(result[i]), result[i][maxIndex])) #打印结果



if __name__ == "__main__":
    main()
    # test_strings = ["fn你vfe", "wzsdfg", "rqwdeg", "n你kwww", "你dnfnf", "dds你ds", "dsgdsg", "你dsdsd", "sdgsds", "klljd你"]
