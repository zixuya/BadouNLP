...
def build_sample(vocab, sentence_length):
    ...
    x = [random.choice(list(vocab.keys())) for _ in range(len(vocab))]
    x = random.sample(x, sentence_length)
    if "他" not in x:
...
def predict(model_path, vocab_path, input_strings):
    ...
    for i, input_string in enumerate(input_strings):
        print("输入：" ,input_string,"预测类别:"+str(torch.argmax(result[i])),"真实类别:"+str(input_string.index("他"))) #打印结果
        # print("输入：%s, 预测类别：%s, 真实类别:%s" % (input_string, str(result[i]),str(torch.argmax(torch.LongTensor(input_string))))) #打印结果



if __name__ == "__main__":
    # main()
    test_strings = ["fnv他ee", "wz他dfg", "他qwdeg", "ndkww他"]
    # 真实类别 3 2 0 5
