...
def count_parameters(model):
    ...
    # 计算Embedding层参数数量
    total_params += np.prod(model.word_embeddings.shape)
    total_params += np.prod(model.position_embeddings.shape)
    total_params += np.prod(model.token_type_embeddings.shape)
    total_params += np.prod(model.embeddings_layer_norm_weight.shape)
    total_params += np.prod(model.embeddings_layer_norm_bias.shape)
    
    # 计算Transformer层参数数量
    for layer_weights in model.transformer_weights:
        # self attention的各个参数
        total_params += np.prod(layer_weights[0].shape)  # q_w
        total_params += np.prod(layer_weights[1].shape)  # q_b
        total_params += np.prod(layer_weights[2].shape)  # k_w
        total_params += np.prod(layer_weights[3].shape)  # k_b
        total_params += np.prod(layer_weights[4].shape)  # v_w
        total_params += np.prod(layer_weights[5].shape)  # v_b
        total_params += np.prod(layer_weights[6].shape)  # attention_output_weight
        total_params += np.prod(layer_weights[7].shape)  # attention_output_bias
        total_params += np.prod(layer_weights[8].shape)  # attention_layer_norm_w
        total_params += np.prod(layer_weights[9].shape)  # attention_layer_norm_b
        total_params += np.prod(layer_weights[10].shape)  # intermediate_weight
        total_params += np.prod(layer_weights[11].shape)  # intermediate_bias
        total_params += np.prod(layer_weights[12].shape)  # output_weight
        total_params += np.prod(layer_weights[13].shape)  # output_bias
        total_params += np.prod(layer_weights[14].shape)  # ff_layer_norm_w
        total_params += np.prod(layer_weights[15].shape)  # ff_layer_norm_b
    
    # 计算Pooler层参数数量
    total_params += np.prod(model.pooler_dense_weight.shape)
    total_params += np.prod(model.pooler_dense_bias.shape)
    
    return total_params

# 计算参数总量
total_params = count_parameters(db)
print(f"DiyBert模型的总参数数量: {total_params}")
