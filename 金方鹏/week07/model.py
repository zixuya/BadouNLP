...
class TorchModel(nn.Module):
    def __init__(self, config):
        ...
        class_num = config["class_num"] #类别数量
        ...
        self.pooling_style = config["pooling_style"] #池化
...
class CNN(nn.Module):
    def __init__(self, config):
        ...
        pad = int((kernel_size - 1)/2)   # 计算填充大小pad，使得卷积操作后输出的序列长度与输入相同。
                                         # 公式(kernel_size - 1)/2确保卷积核中心对齐输入数据。
