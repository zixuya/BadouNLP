...
bert = BertModel.from_pretrained(r"E:\AI\第六周 语言模型\bert-base-chinese", return_dict=False)
...
class DiyBert:
    ...
    def param_counts(self):
        ...
        pooler_count = 0
        # embedding层
        embedding_count += (self.word_embeddings.size+self.position_embeddings.size+self.token_type_embeddings.size+
                  self.embeddings_layer_norm_weight.size+self.embeddings_layer_norm_bias.size)
        # 单个transformer层
        #print(self.transformer_weights[0])
        for weight in self.transformer_weights[0]:
            #print(weight.shape)
            transformer_count += weight.size
        # pooler层
        pooler_count += (self.pooler_dense_weight.size+self.pooler_dense_bias.size)
        count += embedding_count + transformer_count * self.num_layers + pooler_count
        print(count, embedding_count, transformer_count, pooler_count)
        return count
