...
class LanguageModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        mask[:, max_title_length:, max_title_length:] = torch.tril(torch.ones(max_content_length, max_content_length, device=x.device))
...
# def build_sample(tokenizer, window_size, corpus):   # 数据改成两段，标题和内容
#     number = random.randint(0, len(corpus) - 1 )

#     window = corpus[number]['title'][:20]
#     target = corpus[number]['content']

#     x = tokenizer.encode(window, add_special_tokens=False, padding='max_length', truncation=True, max_length=20)   #将字转换成序号
#     y = tokenizer.encode(target, add_special_tokens=False, padding='max_length', truncation=True, max_length=100)
...
if __name__ == "__main__":
    ...
    train(r"D:\python_project_git\ai_study\week11 大语言模型相关第一讲\week11 大语言模型相关第一讲\week11\sample_data.json", False)
