...
bert = BertModel.from_pretrained(r"D:\python\week6 语言模型和预训练\下午\bert-base-chinese", return_dict=False)
...
class DiyBert:
    ...
    def count_params(self):
        ...
        total_params += self.token_type_embeddings.size
        total_params += self.embeddings_layer_norm_weight.size
        total_params += self.embeddings_layer_norm_bias.size
        # transformer部分
        for layer_weights in self.transformer_weights:
            for weight in layer_weights:
                total_params += weight.size
        # pooler层
        total_params += self.pooler_dense_weight.size
        total_params += self.pooler_dense_bias.size
        return total_params
...
# 计算并打印参数数量
total_params = db.count_params()
print(f"自定义Bert模型的可训练参数数量为: {total_params}")
