...
class DataGenerator:
        ...
        with open(self.path, encoding="utf8") as f:
            ...
            for segment in segments:
                ...
                for line in segment.split("\n"): # 换行切分每个字和标签
                    ...
                    labels.append(self.schema[label])  # label转成数字
                self.sentences.append("".join(sentenece))
                input_ids = self.encode_sentence(sentenece)
                labels = self.padding(labels, -1)   # 如果使用bert，一句话内会有 cls sep，所以标签需要补齐
    ...
    def encode_sentence(self, text, padding=True):
        ...
        # 获取 token ID
        input_id = encoding['input_ids'].squeeze(0)  # 因为返回的是 batch_size=1，去掉多余的维度
