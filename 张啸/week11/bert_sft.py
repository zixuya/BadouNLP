...
# 加载语料，用title当成prompt，让其预测content
...
def build_dataset(tokenizer, corpus, max_length, batch_size):
    ...
    for i, (prompt, answer) in enumerate(corpus):
        prompt_encoder = tokenizer.encoder(prompt, add_special_tokens=False)
        answer_encoder = tokenizer.encoder(answer, add_special_tokens=False)
        x = [tokenizer.cls_token_id] + prompt_encoder + [tokenizer.sep_token_id] + answer_encoder + [
            tokenizer.sep_token_id]
        y = len(prompt_encoder) * [-1] + [-1] + answer_encoder + [tokenizer.sep_token_id] + [-1]
        # 构建mask矩阵
        mask = create_mask(len(prompt_encoder), len(answer_encoder))
        # padding
...
def create_mask(s1, s2):
    ...
    for i in range(len_s1):
        # s1当前token不能看到s2的任何token
        mask[i, len_s1:] = 0
    # 遍历s2的每个token
    for i in range(len_s2):
        # s2当前token不能看到后面s2的token
        mask[len_s1 + i, len_s2 + i + 1:] = 0
    return mask
...
def generate_sentence(openings, model, tokenizer):
    model.eval()
    openings = tokenizer.encoder(openings)
    with torch.no_grad():
        # 生成文本超过30则终止迭代
