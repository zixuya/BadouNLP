...
class LanguageModel(nn.Module):
    ...
    def forward(self, x, y=None):
        if y is not None:
            ...
            # x, _ = self.bert(x)有问题，因为bert输出是一个元组，第一个元素是输出，第二个元素是pooler_output，
            # 如果pooler_output为空，就会导致 x 变成 None 或 str 类型，这会导致 self.classify(x) 
...
def build_sample(tokenizer, window_size, corpus):
    ...
    # print(f"build_sample - x: {type(x)}, y: {type(y)}")  # 添加调试信息

    return x, y


# 建立数据集
# sample_length 输入需要的样本数量
# vocab 词表
# window_size 样本长度
# corpus 语料
def build_dataset(sample_length, tokenizer, window_size, corpus):
    ...
    # print(f"build_dataset - x: {type(dataset_x)}, y: {type(dataset_y)}")  # 添加调试信息
...
def train(corpus_path, save_weight=True):
    ...
    for epoch in range(epoch_num):
        ...
        for batch in range(int(train_sample / batch_size)):
            x, y = build_dataset(batch_size, tokenizer, window_size, corpus)    # 构建数据集
            if torch.cuda.is_available():
                x = x.cuda()
                y = y.cuda()   
