...
# 文本生成测试代码
def generate_sentence(openings, model, tokenizer, window_size, temperature):
    ...
    with torch.no_grad():
        ...
        while pred_char != "\n" and len(openings) <= 30:
            ...
            m_cuda_flag = torch.backends.mps.is_available()
            if torch.cuda.is_available():
                x = x.cuda()
            elif m_cuda_flag:
                x = x.to(torch.device('mps'))
            y = model(x)[0][-1]
            index = sampling_strategy(y, temperature)
            # pred_char = reverse_vocab[index]
            pred_char = ''.join(tokenizer.decode(index))
    return openings


def sampling_strategy(prob_distribution, temperature):
...
def train(corpus_path, bert_path, save_weight=True):
    ...
    # char_dim = 768  # 每个字的维度  hidden_size
    window_size = 10  # 样本文本长度
    # vocab_size = 21128  # 字表大小
    learning_rate = 0.001  # 学习率
    temperature = 0.1  # 采样温度 值越小创造力越小
    ...
    for epoch in range(epoch_num):
        ...
        print(generate_sentence("让他在半年之前，就不能做出", model, tokenizer, window_size, temperature))
        print(generate_sentence("李慕站在山路上，深深的呼吸", model, tokenizer, window_size, temperature))
    if not save_weight:
        return
    else:
...
if __name__ == "__main__":
    ...
    train("corpus.txt", bert_path_chinese, True)
