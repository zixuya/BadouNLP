...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        self.activation = nn.Softmax(dim=0)  # 添加正确的激活函数
        self.loss = nn.CrossEntropyLoss()  #

    # 当输入真实标签，返回loss值；无真实标签，返回预测值
    def forward(self, x, y=None):
        x = self.linear(x)  # (batch_size, input_size) -> (batch_size, 1)
        y_pred = self.activation(x)  # 添加激活函数的应用
        if y is not None:
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            y_p = torch.argmax(y_p)  # 获取预测的类别
            y_t = torch.argmax(y_t)  # 获取真实的类别
            if y_p == y_t:
                correct += 1  # 判断正确
            else:
                wrong += 1
...
def main():
    ...
    train_sample = 5000  # 每轮训练总共训练的规律相关样本总数
    input_size = 5  # 输入向量维度
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        y_true = np.zeros(5)
        y_true[np.array(vec).argmax()] = 1
        y_pred = np.zeros(5)
        y_pred[res.argmax()] = 1
        print("输入：%s,真实类别：%s 预测类别：%s, 概率值：%s" % (vec, y_true, y_pred, res))  # 打印结果
