...
def build_dataset(data, tokenizer, max_title_length, max_content_length):
    ...
    for d in data:
        ...
        x = tokenizer.encode(x_, add_special_tokens=False, padding='max_length', truncation=True,
                             max_length=max_length)  # 将字转换成序号
...
def train(corpus_path, save_weight=True):
    ...
    for epoch in range(epoch_num):
        ...
        for batch in range(int(train_sample / batch_size)):
            x, y = dataset[0][batch * batch_size: (batch + 1) * batch_size], dataset[1][batch * batch_size: (
                                                                                                                        batch + 1) * batch_size]
        ...
        print(generate_sentence("世界最大胸罩将拍卖 面积堪比两个网球场", model, tokenizer))
