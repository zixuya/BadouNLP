...
class DataGenerator:
    ...
    def random_train_sample(self):
        ...
        if random.random() <= self.config["positive_sample_rate"]:
            # Sample a negative class different from p
            p_negative = random.choice([q for q in standard_question_index if q != p])
            s3= random.choice(self.knwb[p_negative])
        else:
            # Ensure s3 is different from both p and s2
            p_negative = random.choice([q for q in standard_question_index if q != p])
            s3 = random.choice(self.knwb[p_negative])
