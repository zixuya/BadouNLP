...
class LanguageModel(nn.Module):
    def __init__(self, vocab):
        ...
        self.bert = BertModel.from_pretrained("D:/NLP/pretrain_model/bert-base-chinese")
    ...
    def forward(self, x, mask, y=None):
        x, _ = self.bert(x, attention_mask = mask, return_dict = False)        #output shape:(batch_size, sen_len, input_dim)
...
def build_dataset(sample_length, vocab, window_size, corpus):
    ...
    return torch.LongTensor(dataset_x), torch.LongTensor(dataset_y), torch.stack(attention_mask)
...
def train(corpus_path, save_weight=True):
    ...
    if torch.cuda.is_available():
        model = model.cuda()
    optim = torch.optim.Adam(model.parameters(), lr=0.00002)   #建立优化器
    ...
    for epoch in range(epoch_num):
        ...
        print(generate_sentence("等过些日子回了神都，和女皇", model, vocab, window_size))
        print(generate_sentence("李慕是无神论者，但也看过", model, vocab, window_size))
    if not save_weight:
        return
    else:
