...
#最终预测
def calc_weight_num(model):
    total = sum(p.numel() for p in model.parameters())
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print("总参数数量：%d, 可训练参数数量：%d" % (total, trainable))
    # 换算成十亿（B）
    total_b = total / 1_000_000_000
    trainable_b = trainable / 1_000_000_000

    print("总参数数量：%.2fB, 可训练参数数量：%.2fB" % (total_b, trainable_b))
