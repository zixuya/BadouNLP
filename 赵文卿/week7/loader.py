'''
Author: Zhao
Date: 2025-01-08 18:09:17
LastEditTime: 2025-01-09 15:50:22
FilePath: loader.py
Description: 数据加载

'''
import json
import re
import os
import torch
import numpy as np
from torch.utils.data import Dataset, DataLoader
from transformers import BertTokenizer

class DataGenerator:
    def __init__(self,data_path,config):
        self.config = config
        self.path = data_path
        # 定义标签和索引的映射关系
        self.index_to_label = {0: '家居', 1: '房产', 2: '股票', 3: '社会', 4: '文化',
                               5: '国际', 6: '教育', 7: '军事', 8: '彩票', 9: '旅游',
                               10: '体育', 11: '科技', 12: '汽车', 13: '健康',
                               14: '娱乐', 15: '财经', 16: '时尚', 17: '游戏'}
        self.label_to_index = dict((y,x) for x,y in self.index_to_label.items())
        self.config["class_num"] = len(self.index_to_label)     # 设置类别数量
        # 加载 BertTokenizer，如果模型类型为 BERT
        if self.config["model_type"] == "bert":
            self.tokenizer = BertTokenizer.from_pretrained(config["pretrain_model_path"])
        # 加载词汇表并设置词汇表大小
        self.vocab = load_vocab(config["vocab_path"])
        self.config["vocab_size"] = len(self.vocab)
        # 加载数据
        self.load()

    def load(self):
        self.data = []
        with open(self.path, encoding="utf8") as f:
            for line in f:
                line = json.loads(line) # 解析 JSON 行
                tag = line["tag"]
                label = self.label_to_index[tag]
                title = line["title"]
                # 编码输入句子，根据模型类型选择不同的编码方式
                if self.config["model_type"] == "bert":
                    input_id = self.tokenizer.encode(title, max_length=self.config["max_length"], padding='max_length', truncation=True)
                    #input_id = self.tokenizer.encode(title, max_length=self.config["max_length"], pad_to_max_length=True)
                else:
                    input_id = self.encode_sentence(title)
                input_id = torch.LongTensor(input_id)
                label_index = torch.LongTensor([label])
                self.data.append([input_id,label_index])
        return
    
    def encode_sentence(self, text):
        input_id = []
        for char in text:
            input_id.append(self.vocab.get(char, self.vocab["[UNK]"])) # 获取字符的索引，找不到时用 [UNK] 代替
        input_id = self.padding(input_id) # 对序列进行填充
        return input_id
    
    def padding(self, input_id):
        # 对序列进行填充或截断，确保长度为 max_length
        input_id = input_id[:self.config["max_length"]]
        input_id += [0] * (self.config["max_length"] - len(input_id))
        return input_id
    
    def __len__(self):
        return len(self.data) # 返回数据集的大小
    
    def __getitem__(self, index):
        return self.data[index] # 根据索引返回数据项

def load_vocab(vocab_path):
    token_dict = {}
    with open(vocab_path, encoding="utf8") as f:
        for index, line in enumerate(f):
            token = line.strip()
            token_dict[token] = index + 1  #0留给padding位置，所以从1开始
    return token_dict



def load_data(data_path, config, shuffle = True):
    """
    shuffle 参数来控制是否打乱数据
    DataLoader为模型训练和测试提供了一种方便的方法来批量加载数据，并进行一些有用的操作
    """
    dg = DataGenerator(data_path,config)
    dl = DataLoader(dg, batch_size=config["batch_size"], shuffle=shuffle)
    return dl

# if __name__ == "__main__":
#     from config import Config
#     Config["max_length"] = 4
#     Config["model_type"] = "cnn"
#     train_data_path = "week7/data/train_tag_news.json" 
#     valid_data_path = "week7/data/valid_tag_news.json" 
#     test_path = "week7/data/test_file.json" 
#     dg = load_data(valid_data_path,Config)
#     print(dg)